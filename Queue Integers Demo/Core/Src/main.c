...
/* Producer Task: sends incrementing numbers */
void ProducerTask(void *pvParameters)
{
  uint32_t count = 0;
  char msg[50];
  ...
  {
    count++;
    xQueueSend(QueueHandle, &count, portMAX_DELAY); // put into queue

    sprintf(msg, "Produced: %lu\r\n", count);
    HAL_UART_Transmit(&huart1, (uint8_t*)msg, strlen(msg), HAL_MAX_DELAY);

    vTaskDelay(pdMS_TO_TICKS(1000)); // produce every 1 sec
  }
}

/* Consumer Task: receives and prints numbers */
void ConsumerTask(void *pvParameters)
{
  uint32_t value;
  char msg[50];
  ...
  for (;;)
  {
    ...
    {
      sprintf(msg, "Consumed: %lu\r\n", value);
      HAL_UART_Transmit(&huart1, (uint8_t*)msg, strlen(msg), HAL_MAX_DELAY);
    }
  }
}

/* --- HAL init functions (unchanged from CubeMX) --- */
